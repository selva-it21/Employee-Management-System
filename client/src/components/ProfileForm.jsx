...
const ProfileForm = ({ initialData, onSuccess }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className='card p-5 sm:p-6 mb-6 mt-6'>
            ...
            <div className='space-y-5'>
                ...
                <div>
                    ...
                    <textarea disabled={initialData.isDeleted} name="bio" id="" defaultValue={initialData.bio || ""} placeholder='Write a brief bio...' className={`resize-none ${initialData.isDeleted ? "bg-slate-50 text-slate-400 cursor-not-allowed" : ""}`}></textarea>
                    <p className='text-xs text-slate-400 mt-1.5'> This will be displayed on your profile.</p>
                </div>
                {initialData.isDeleted ? (
                    <div className='pt-2'>
                        <div className='p-4 bg-rose-50 border border-rose-200 rounded-sx text-center'>
                    ...
                    <div  className='flex justify-end pt-2'>
                        <button type='submit' className='btn-primary flex items-center gap-2 justify-center w-full sm:w-auto' disabled={loading}>
                           {loading ? <Loader2 className='w-4 h-4'/> : <Save className='w-4 h-4'/>} Save Changes
                        </button>
                    </div>
                )}
            </div>

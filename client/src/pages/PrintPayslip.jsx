  ...
  return (
    <div className='max-w-2xl mx-auto p-8 bg-white animate-fade-in'>
      ...
      <div className='grid grid-cols-2 gap-6 mb-8'>
        <div>
          <p className='text-sx text-slate-400 uppercase tracking-wider mb-1'>Employee Name</p>
          <p className='font-semibold text-slate-900'>{payslip.employee?.firstName} {payslip.employee?.lastName}</p>
        </div>
        <div>
          <p className='text-sx text-slate-400 uppercase tracking-wider mb-1'>Position</p>
          <p className='font-semibold text-slate-900'>{payslip.employee?.position}</p>
        </div>
        <div>
          <p className='text-sx text-slate-400 uppercase tracking-wider mb-1'>Email</p>
          <p className='font-semibold text-slate-900'>{payslip.employee?.email}</p>
        </div>
        <div>
          <p className='text-sx text-slate-400 uppercase tracking-wider mb-1'>Period</p>
        ...
        <table className='w-full text-sm'>
          ...
          <tbody>
            <tr className='border-t border-slate-100'>
              ...
              <td className='py-3 px-4 text-slate-900 font-medium'>${payslip.basicSalary?.toLocaleString()}</td>
            </tr>
            <tr className='border-t border-slate-100'>
              <td className='py-3 px-4 text-slate-700'>Allowances</td>
              <td className='py-3 px-4 text-slate-900 font-medium'>+${payslip.allowances?.toLocaleString()}</td>
            </tr>
            <tr className='border-t border-slate-100'>
              <td className='py-3 px-4 text-slate-700'>Deductions</td>
              <td className='py-3 px-4 text-slate-900 font-medium'>-${payslip.deductions?.toLocaleString()}</td>
            </tr>
            <tr className='border-t-2 border-slate-200 bg-slate-50'>
              <td className='py-4 px-4 font-bold text-slate-900'>Net Salary</td>
              <td className='py-4 px-4 font-bold text-slate-900 text-lg'>+${payslip.netSalary?.toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
